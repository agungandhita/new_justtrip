...
// Popular Packages Tabs logic - filter by jenis_layanan
const openTripPackages = computed(() => props.popularPackages?.filter((p) => p.jenis_layanan === 'open_trip') || []);
const privateTripPackages = computed(() => props.popularPackages?.filter((p) => p.jenis_layanan === 'private_trip') || []);
const requestTripPackages = computed(() => props.popularPackages?.filter((p) => p.jenis_layanan === 'request_trip') || []);
...
<template>
    <FrontendLayout title="Welcome">
            ...
            <div class="relative z-20 w-full px-4 max-w-7xl mx-auto pt-20 md:pt-0">
                <div class="max-w-4xl mx-auto text-center mb-4 md:mb-10">
                    ...
                    <div class="mt-6 flex flex-wrap gap-3 justify-center" data-aos="zoom-in" data-aos-delay="400">
                        <Link href="/promo" class="group relative px-5 sm:px-8 py-2.5 sm:py-3.5 bg-orange-500 rounded-xl text-white font-bold text-sm sm:text-base transition-all duration-500 hover:bg-orange-600 hover:shadow-[0_0_20px_rgba(249,115,22,0.4)] overflow-hidden">
                            <span class="relative z-10 flex items-center">
                                <i class="fas fa-tags mr-2 group-hover:rotate-12 transition-transform"></i>
                                Promo Spesial
                            </span>
                        </Link>
                        <Link href="/paket-wisata" class="px-5 sm:px-8 py-2.5 sm:py-3.5 bg-white/10 backdrop-blur-md border border-white/30 rounded-xl text-white font-bold text-sm sm:text-base hover:bg-white hover:text-blue-900 transition-all duration-500 flex items-center">
        ...
        <section v-if="reviews && reviews.length > 0" id="customer-reviews" class="py-12 sm:py-16 md:py-20 bg-gradient-to-br from-teal-50 to-cyan-50">
            <div class="container mx-auto px-4 max-w-7xl">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
                    <div v-for="(review, index) in reviews" :key="review.id ?? index" class="group bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 overflow-hidden" data-aos="fade-up" :data-aos-delay="(index + 1) * 100">
                        <!-- Card Content -->
                        <div class="p-6 sm:p-8">
                            ...
                            <div class="mb-4 text-yellow-400 text-lg sm:text-xl">
                                <span v-for="n in (review.rating ?? 5)" :key="n">★</span>
                            </div>

                            <!-- Review Content -->
                            <p class="text-gray-700 text-sm sm:text-base leading-relaxed mb-6 line-clamp-4">"{{ review.komentar }}"</p>

                            <!-- Customer Info -->
                            <div class="flex items-center border-t border-gray-100 pt-6">
                                <img :src="review.foto_reviewer ?? `https://ui-avatars.com/api/?name=${encodeURIComponent(review.nama_reviewer)}`" :alt="review.nama_reviewer" loading="lazy" class="w-12 h-12 sm:w-14 sm:h-14 rounded-full object-cover border-2 border-teal-100 mr-4 group-hover:border-teal-400 transition-colors">
                                <div class="flex-1 min-w-0">
                                    <h4 class="font-bold text-gray-800 text-sm sm:text-base truncate">{{ review.nama_reviewer }}</h4>
                                    <p v-if="review.destinasi" class="text-gray-500 text-xs sm:text-sm truncate">{{ review.destinasi }}</p>
                                </div>
                            </div>
