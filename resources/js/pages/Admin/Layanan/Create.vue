...
const form = useForm({
    ...
    itinerary: [] as { hari: number, judul: string, deskripsi: string }[],
    fasilitas_include: '',
    fasilitas_exclude: '',
    catatan_penting: '',
...
const previewUtama = ref<string | null>(null)
const previewDestinasi = ref<{ file: File, url: string }[]>([])
...
const removeDestinasi = (index: number) => {
    form.gambar_destinasi.splice(index, 1)
...
const removeItineraryDay = (index: number) => {
    ...
    // Update the 'hari' property for remaining items
    form.itinerary.forEach((item, idx) => { item.hari = idx + 1 })
    form.durasi_hari = form.itinerary.length.toString()
}

function submit() {
    form.post('/admin/layanan')
}
</script>
